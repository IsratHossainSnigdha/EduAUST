...
export default function StudentDashboard({ darkMode, toggleDarkMode }) {
   ...
   * a tutor profile before switching dashboards.
   */
  const handleSwitchToTutor = async () => {
    try {
      const { ok, body } = await apiGet('/tutor/status');

      if (!ok) {
        console.error('Failed to check tutor status.');
        return;
      }
      ...
      if (body?.isTutor) {
        ...
        // User does not have a tutor profile yet
  ...
  /*
   * Badge count for this dashboard only;
   * the API reports each side separately.
   */
  useEffect(() => {
    let cancelled = false;
    ...
    >
      ...
      <main className="flex-grow p-6 lg:p-10 space-y-8 overflow-y-auto max-h-screen">
        ...
        <div className="space-y-1">
          ...
          >
            Let's Connect, Ishrat!{' '}
            <span className="animate-bounce">👋</span>
          </h1>

          <p className={`text-xs sm:text-sm ${textSecondary}`}>
            Find the right tutor and ace your studies.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
          {[
            {
              title: 'Find Tutors',
              desc: 'Search by course or subject',
              icon: Search,
              color:
            ...
            {
              title: 'Saved Tutors',
              desc: 'View your saved tutor list',
              icon: Heart,
              color:
                'text-pink-500 bg-pink-500/10 border-pink-500/15',
              path: '#',
            },
            {
              title: 'My Requests',
            ...
            {
              title: 'Messages',
              desc: 'Chat with active tutors',
              icon: MessageSquare,
              color:
                'text-violet-500 bg-violet-500/10 border-violet-500/15',
              badge: 3,
              path: '/messages',
            },
          ].map((card, idx) => {
              ...
              <div className="space-y-3">
                {[
                  ...
                  >
                    ...
                    <span
                      className={`text-[9px] font-extrabold px-2 py-0.5 rounded border ${req.statusColor}`}
                    >
                      {req.status}
                    </span>
                  </div>
