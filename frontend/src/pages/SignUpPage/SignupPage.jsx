...
export default function SignUpPage({
  ...
  // Step 1 account details + API state
  ...
  const handleVerifyCode = async () => {
    ...
    setLoading(true);
    const { ok, body } = await postJson('/auth/register/verify', {
    ...
    if (!ok) {
      setOtpError(firstError(body, 'The verification code is incorrect.'));
      return;
    }
  ...
  const handleResend = async () => {
    ...
    setOtpMessage('');
    setLoading(true);
    const { ok, body } = await postJson('/auth/register/resend', {
      registration_token: registrationToken,
    });
    setLoading(false);

    if (!ok) {
      setOtpError(firstError(body, 'Could not resend the code.'));
      return;
    }
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    setLoading(true);
    const { ok, body } = await postJson('/auth/register/security', {
      registration_token: registrationToken,
      password,
    ...
    if (!ok) {
      setFormError(firstError(body, 'Could not create your account. Please try again.'));
      return;
    }
      ...
      <div className={`w-full max-w-lg p-10 rounded-3xl border shadow-xl relative transition-all duration-300 ${cardClass}`}>
          ...
          <div className="pt-4">
            ...
            <form onSubmit={handleAccountInfoSubmit} className="space-y-5">
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Ishrat Jahan Ifa" className={`w-full px-5 py-3.5 rounded-xl border focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition duration-200 ${inputBgClass} ${textColor}`} required />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={departmentId} onChange={(e) => setDepartmentId(e.target.value)} className={`w-full px-5 py-3.5 rounded-xl border focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition duration-200 ${inputBgClass} ${textColor}`} required>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                        ...
                        if (v.length >= 2 && !v.startsWith('01')) v = '01';
                        setPhone(v);
                    }}
                    className={`w-full px-5 py-3.5 rounded-xl border focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition duration-200 ${inputBgClass} ${textColor}`}
                    required
        ...
        {signUpStep === 3 && (
          <div className="text-center pt-4">
            ...
            {otpError && <p className="text-sm text-red-500 font-medium mb-3">{otpError}</p>}
            {otpMessage && <p className="text-sm text-emerald-600 font-medium mb-3">{otpMessage}</p>}

            <button onClick={handleVerifyCode} disabled={loading} className="w-full py-4 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white font-bold rounded-xl transition shadow-md shadow-emerald-600/10 text-base">{loading ? 'Verifying…' : 'Verify & Proceed'}</button>
            <p className="text-xs mt-4">Didn't receive code? <button type="button" onClick={handleResend} disabled={loading} className="text-emerald-600 font-semibold hover:underline disabled:opacity-60">Resend Code</button></p>
          </div>
        )}
        ...
        {signUpStep === 5 && (
          <div className="pt-4">
            ...
            <form onSubmit={handlePasswordSubmit} className="space-y-5">
              ...
              {formError && <p className="text-sm text-red-500 font-medium text-center">{formError}</p>}
              <button type="submit" disabled={!agreeTerms || loading} className={`w-full mt-6 py-4 text-white font-bold rounded-xl transition-all duration-200 text-base ${agreeTerms && !loading ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-slate-300 cursor-not-allowed'}`}>{loading ? 'Creating account…' : 'Create Account'}</button>
            </form>
          </div>
        )}
